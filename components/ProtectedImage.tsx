...
export function ProtectedImage({
  ...
  return (
    <figure
      className={`group relative mb-3 break-inside-avoid cursor-pointer transition-opacity duration-150 md:mb-4 ${
    ...
    >
      <div className="relative bg-gray-200" style={{ aspectRatio: `${photo.aspectRatio}` }}>
        ...
        <button
          type="button"
          className="absolute left-2 top-2 font-mono text-[10px] uppercase tracking-[0.12em] text-white opacity-0 transition-opacity duration-75 group-hover:opacity-100"
          onClick={toggleSaved}
        >
          {isSaved ? '[ SAVED ]' : '[ SAVE ]'}
        </button>
        ...
        {/* View hint */}
        <span className="absolute bottom-2 right-2 font-mono text-[10px] uppercase tracking-[0.12em] text-white opacity-0 transition-opacity duration-75 group-hover:opacity-100">
